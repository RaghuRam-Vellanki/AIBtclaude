"""
strategies/nifty_microstructure.py
Citadel Securities-style mean reversion for NIFTY 50: when ^NSEI extends
>2σ from session VWAP and the most recent 1m bars show wick rejection
(long lower wicks at a bottom, long upper wicks at a top), fade the move.
"""
...
class NIFTYMicrostructureStrategy(StrategyAgent):
    name = "nifty_microstructure"
    ...
    def vote(self, snapshot: Dict[str, Any], feed: Any) -> StrategyVote:
        try:
            ...
            if zscore <= -2.0 and mean_wick < -0.30:
                ...
                return StrategyVote(
                    ...
                    rationale=(
                        f"Z={zscore:.2f}σ below VWAP with lower-wick rejection "
                        f"(wick imbalance {mean_wick:+.2f}) — fade the dump"
                    ),
            ...
            if zscore >= 2.0 and mean_wick > 0.30:
                ...
                return StrategyVote(
                    ...
                    confidence=confidence,
                    rationale=(
                        f"Z={zscore:.2f}σ above VWAP with upper-wick rejection "
                        f"(wick imbalance {mean_wick:+.2f}) — fade the spike"
                    ),
                    metadata={"zscore": round(zscore, 2), "wick_imbalance": round(mean_wick, 2)},
                )
